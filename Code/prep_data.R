...
# Section 3: test matches for a given country
  
  test_country = "Cabo Verde"
  
  test = full_join(filter(trust_by_subregion, COUNTRY == test_country), 
...
# Section 6: combine everything into final datasets
    ...
    # add in population data
      ...
      pop_subregion = read_excel(paste0(input_path, "population_subregion.xlsx")) %>% 
                      filter(used == 1) %>% 
                      select(-c(source, urban_x, used, issue_movement, misspelling_schleifer, ADM0_CODE, ADM1_CODE, issue_adm1, unique_date)) %>% 
    ...
    # generate final datasets for different timeframes
      ...
      for (i in 1:4){
        ...
        assign(dataset_names[i],
               ...
               left_join(GDP_sub, by = c("country" = "Country", "region" = "Region")) %>% 
               ...
               filter(is.na(arith_change) == FALSE) %>% 
               arrange(country, region) 
               )
      }
      
      
    # generate final datset that excludes Mauritius
